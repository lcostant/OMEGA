...
This file software allows to create RGB NIRCam-like postage stamps for IllustrisTNG50 galaxies.
...
def filter_selection(i_a, filter):
	...
	fits_image_filename = f'{abs_path}/{args.id}_{i_a}_z{args.redshift}_TNG50.fits'
...
if __name__ == "__main__":	
	...
	parser = argparse.ArgumentParser(description='Create RGB NIRCam synthetic images of IllustrisTNG50 galaxies')
	parser.add_argument("--id", help='"ID of the galaxy (default: 188916)"', nargs='?', type=int, const=188916, default=188916)
	parser.add_argument("--redshift", help='"redshift of the galaxy (default: 3)"', nargs='?', type=int, choices=range(3,7), const=3, default=3)
	...
	image_r, wcs_r = filter_selection(i_a=i_a, filter=args.f1)
	image_g, wcs_g = filter_selection(i_a=i_a, filter=args.f2)
	image_b, wcs_b = filter_selection(i_a=i_a, filter=args.f3)
